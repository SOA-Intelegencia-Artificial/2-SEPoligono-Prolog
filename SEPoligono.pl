...
ask(Pred, Pregunta, X):-write(Pregunta),read(Y),asserta(memory(Pred, Y)),X == Y.
...
nombre(trianguloRectangulo):-nombre(triangulo),anguloRecto(oui).
...
nombre(diamante):-nombre(paralelogramo),ladosIguales(4).

nombre(cuadrado):-nombre(diamante),nombre(rectangulo).